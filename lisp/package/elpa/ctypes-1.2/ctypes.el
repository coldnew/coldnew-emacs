...
;; Copyright (C) 1997 Anders Lindgren.
...
;;     hello(foo * bar);
;;
;; a) A new function `hello' that takes one argument `bar' that is a
;; pointer to a `foo', or;
;;
;; b) call the function `hello' with the result of `foo' multiplied
;; by `bar'.
;;
...
;; The future:
;;
;; Should this package be included in Emacs 19.35 almost all of the
;; font-lock code could be removed.  Also there will be no need to
;; load font-lock to determine which version of ctypes-mode-descriptor
;; to use.
...
(defvar ctypes-mode-descriptor
  (if (boundp 'c-font-lock-extra-types)
      ;; This version of the mode descriptor needs font-lock from
      ;; Emacs 19.35.
      '((c-mode
...
;; TODO: classes need not start at the beginning of a line, do they?
(defun ctypes-parse-buffer-c++ (&optional filename)
  ...
  (let ((orig-syntax-table (syntax-table)))
    ...
    (unwind-protect
	(let ((lst '()))
	  (while (re-search-forward "^\\(\\(typedef\\)\\|class\\)\\>" nil t)
	    (condition-case ()
...
(or (fboundp 'font-lock-keyword-depth)
...
(defun ctypes-font-lock-set-extra-types (mode extra-types-var)
  "Set a Emacs 19.35 (not yet released) style font-lock regexp."
	  ...
	  (list 'font-lock-match-c++-style-declaration-item-and-skip-to-next
	    ...
	    (list 'goto-char 
		  (list 'or (list 'match-beginning
				  (+ 2 (font-lock-keyword-depth regexp)))
			'(match-end 1)))
