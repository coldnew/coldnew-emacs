...
#+begin_src emacs-lisp
  ...
  (add-hook 'egg-commit-buffer-mode-hook
            '(lambda ()
               ;;(define-key coldnew/command-mode-map "c" 'egg-log-msg-done)
               (set (make-local-variable 'egg:command-mode) t)
               (call-interactively egg:command-mode)
               ))
...
#+begin_src emacs-lisp
  ...
  (defun coldnew/parse-keymap (s)
    (let ((ss (car-safe (string-to-list s))))
      (cond ((= ?\^x ss)
             (concat  "Ctrl-x" (list-to-string (cdr-safe (string-to-list s)))))
            )))
  ...
  (concat "C-x" (list-to-string (cdr (string-to-list "\C-x e"))))
