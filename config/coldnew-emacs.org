...
** List Processing
*** flatten a list
flatten a list 是一種方法，可以用來將巢狀的 list 變承單一個 list。
舉例來說，假如目前有這樣一個 list:
  ...
  (defun coldnew/test3 (s)
    (let* ((char-list (string-to-list (replace-regexp-in-string " " "" s)))
      ...
      (dolist (char char-list)
        ...
        (if (assoc char coldnew/keymap-string-alist)
            (setq char (reverse (string-to-list (cdr-safe (assoc char coldnew/keymap-string-alist))))))
        (setq result-list (cons (list char) result-list))
        (setq result-list  (cons (list ?\s) result-list))
        )
      ;;result-list
      (setq result-list (flatten result-list))
