...
#+begin_src emacs-lisp
  (defun create-tags (regexp)
    "Create TAGS file in current directory."
    (interactive "sRegexp: *.[ch] : ")
    (if (string-equal "" regexp) (setq regexp "*.[ch]"))
    (eshell-command
     (format "find . -type f -name \"%s\" | etags -" regexp)))
#+end_src
...
#+begin_src emacs-lisp
  (vim:imap (kbd "C-x q") 'toggle-kbd-macro-recording-on)
  (vim:imap (kbd "C-x e") 'call-last-kbd-macro)
;;  (vim:mode-map coldnew-editor-map (kbd "C-x i") 'call-last-kbd-macro)
#+end_src
...
#+begin_src emacs-lisp
  (vim:map (kbd "C-c C-e") 'eval-and-replace)
  (vim:map (kbd "C-x l") 'hl-line-mode)
  (vim:map (kbd "C-x L") 'linum-mode)
#+end_src
...
#+begin_src emacs-lisp
  (vim:map (kbd "C-x C-f")  'lusty-file-explorer)
  (vim:map (kbd "C-x C-r")  'lusty-sudo-explorer)
  (vim:map (kbd "C-x C-s")  'save-buffer-always)
#+end_src
...
#+begin_src emacs-lisp
  (vim:imap (kbd "C-x C-y")  'yank-n-times)
  (vim:imap (kbd "C-x C-l")  'recenter)
#+end_src
