...
#+begin_src python :tangle no
  ...
  def get_password_emacs(machine):
      cmd = "emacsclient --eval '(offlineimap-read-authinfo \"%s\")'" % (machine)
...
#+begin_src emacs-lisp
  (defun offlineimap-read-authinfo (machine)
      "A simple implement to let offlinemap read my authinfo password"
      (let  ((authinfo (netrc-machine (netrc-parse "~/.authinfo.gpg") machine)))
        (netrc-get authinfo  "password")))
#+end_src
...
#+begin_src emacs-lisp
  (dolist
      (r `(
           ...
           (?n (file . "~/Dropbox/Note/Note.org"))
           (?b (file . "~/Workspace/Blog/blog.org"))
           ))
...
*** 設定 blog 檔案的位置
#+begin_src emacs-lisp
  (defvar coldnew-blog-org-file "~/Workspace/Blog/blog.org"
    "coldnew's blog file")
#+end_src
*** 更新 blog 的命令
#+begin_src emacs-lisp
  (defun coldnew/update-blog ()
    (interactive)
    (let* ((dir "~/Workspace/WriteOnce/Blog")
           (cd-cmd (concat "cd " dir ";"))
           (git-remove-cmd (concat cd-cmd "git rm -rf *"))
           (git-add-cmd    (concat cd-cmd "git add ." ";" "git commit -m 'update blog'"))
           (git-push-cmd   (concat cd-cmd "git push origin master")))
    (start-process-shell-command "blog-remove-old" nil git-remove-cmd)
    (org-publish-blog coldnew-blog-org-file)
    (start-process-shell-command "blog-add-new" nil git-add-cmd)
    (start-process-shell-command "blog-update" nil git-push-cmd)
    (firefox-refresh)))
#+end_src
* Local File Variable
# Local Variables:
