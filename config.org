...
#+begin_src emacs-lisp
  ...
  (add-hook 'eshell-output-filter-functions 'coldnew/colorfy-eshell-prompt)
  #+end_src
...
| 視窗切換 | M-[     | 回復上一個窗口佈局                 | 'winner-undo       |
| 視窗切換 | M-]     | 切回上一組進行窗口復原時的窗口佈局 | 'winner-redo       |
