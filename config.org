...
#+BEGIN_SRC emacs-lisp
  (defadvice use-package-ensure-elpa (around package-install (package) activate)
...
#+BEGIN_SRC emacs-lisp
  ...
  This function is used to abstract package loading."
    ;; require feature first, ignore error
    `(require ,feature nil 'noerror)
    ;; use req-pckage to mantain packages
    (eval
...
在 emacs 中有一些內建套件沒有預設載入，這邊將其載入使用。
...
他可以透過許多函式庫來協助你更好的在 emacs 中進行補全。
...
| Shell    | C-x s   | shell-command     | 執行 shell 命令                    |
| 暫存器   | C-x rj  | jump-to-register  | 跳轉到暫存的檔案或是緩衝區位置     |
| 版本管理 | C-x v l | magit-log         | 使用 magit 察看 log                |
| 版本管理 | C-x v s | magit-status      | 使用 magit 查看目前的 git status   |
| 版本管理 | C-x v c | magit-checkout    | 切換到其他的 git branch            |
| 版本管理 | C-x v p | magit-push        | push 到遠端 repo                   |

#+BEGIN_SRC emacs-lisp :noweb yes :results silent
  <<gen-keys(func="global-set-key", keys=global-keys)>>
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :noweb yes :results silent
  <<gen-keys(func="define-key evil-normal-state-map", keys=global-evil-normal)>>
#+END_SRC
