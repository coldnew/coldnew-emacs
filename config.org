...
*** 十進位轉換成十六進位
#+begin_src emacs-lisp
  (defun decimal->hexadecimal (decimal)
...
*** 十六進位轉換成十進位
#+begin_src emacs-lisp
  (defun hexadecimal->decimal (hex)
    "Convert hexdecimal number or string to digit-number."
    (let ((case-fold-search nil)
          (hexstr hex))
      (if (not (stringp hexstr))
          (setq hexstr (symbol-name hex)))
      ;; Use #x as hex prefix (elisp, ...)
      (setq hexstr (replace-regexp-in-string "^#x" "" hexstr))
      ;; Use 0x as hex prefix (C, Perl)
      (setq hexstr (replace-regexp-in-string "^0x" "" hexstr))
      ;; Use # as hex prefix (CSS)
      ;;      (setq hexstr (replace-regexp-in-string "^#"  "" hex ))
      (setq hexstr (string-to-number hexstr 16)))
    (princ hexstr))
#+end_src
** Sort
*** quicksort
...
#+end_src
#+begin_src emacs-lisp
  (defalias 'dec->hex   'decimal->hexadecimal)
  (defalias 'hex->dec   'hexadecimal->decimal)
#+end_src
* Version Control
**  Git
*** egg
