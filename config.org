...
用了 emacs 至今已有 7 年了，最初我的 emacs 配置也是英文版，並且分散了許多檔案，
...
這邊的按鍵設定，刻意讓 eshell 和一般 bash 下使用的形式相同。
...
#+begin_src emacs-lisp
  (let ((org-files (concat coldnew-sparkleshare-dir "/org/")))
    (setq org-directory org-files)
    (setq org-agenda-files (list org-files)))
#+end_src

*** 設定預設的 logdone 為加上時間註記
...
*** 使用 org-reveal 製作網頁投影片

org-reveal 是 reveal.js 的 org-mode wrapper，使用他你必須安裝 8.0 版本以
上的 org-mode。

#+begin_src emacs-lisp
  (if (>= (string-to-number (org-version)) 8.0)
      (require 'ox-reveal))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (add-hook 'newlisp-mode-hook '(lambda () (run-hooks 'lisp-mode-common-hook)))
#+end_src
...
#+begin_src emacs-lisp
  (when (require 'rainbow-delimiters nil 'noerror)
    (add-hook 'lisp-mode-common-hook '(lambda () (rainbow-delimiters-mode 1))))
#+end_src
...
** 當將 Draft 裏面的文章變成 DONE 狀態的時候，移至 Blog 項目

我將我文章分為兩個部份，一個是已經寫好的文章，都放到 Blog 項目裏面，草
稿的部份，則是放到 Draft 裏面，當 Draft 裏面的子項目變成 DONE 的時候，自動將
其移到 Blog 項目內。

#+begin_src emacs-lisp :tangle no
  (defun coldnew/blog-track-draft-done (change-plist)
    "Track Draft task, move it to Blog task if it's stats change to DONE."
    (let* ((pos (plist-get change-plist :position))
          (task (format "** %s\n%s\n" (org-get-heading) (org-get-entry)))
          (blog-task-name  "Blog")
          (draft-task-name "Draft")
          (is-draft-task (member draft-task-name
                                 (save-excursion
                                   (org-up-heading-safe)
                                   (org-heading-components)))))
      (when (and is-draft-task
                 (member (nth 2 (org-heading-components)) org-done-keywords))
        (save-excursion
          (save-restriction
            (widen)
            (goto-char (point-min))
            (when (search-forward-regexp (concat "^* " blog-task-name))
  ...
  (add-hook 'org-trigger-hook 'coldnew/blog-track-draft-done)
#+end_src
