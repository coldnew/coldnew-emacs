...
在這邊我將 doxymacs 的功能加入到所有和程式相關的模式中。
#+BEGIN_SRC emacs-lisp
  (require 'doxymacs)
  (add-hook 'prog-mode-hook '(lambda () (doxymacs-mode)))
#+END_SRC

** string-inflection :: 切換不同變數命名規則幫手
...
GitHub: https://github.com/sabof/es-windows
...
#+TBLNAME: clojure-keys
| 按鍵    | 命令          | 描述                               |
|---------+---------------+------------------------------------|
| C-c M-j | cider-jack-in | 讓 cider 與目前的 clojure 專案連線 |

#+BEGIN_SRC emacs-lisp :noweb yes
  <<gen-keys-map2(map="clojure-mode", keys=clojure-keys)>>
#+END_SRC
...
*** 針對 Linux Kernel 開發，使用標準的 Linux Kernel Coding Style

在 [[https://www.kernel.org/doc/Documentation/CodingStyle][Linux Kernel Coding Style]] 一文中有提到 emacs 的設定，假設打開的程式
碼是屬於 C 語言，並且符合偵測的規則，則將 coding style 切換成文章提及的方
式。
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun coldnew/linux-kernel-source-hook ()
  ...
  ;;(add-hook 'c-mode-hook 'coldnew/linux-kernel-source-hook)
#+END_SRC
...
#+END_SRC
*** 使用 git-messenger 顯示當前行的 commit 訊息

GitHub: https://github.com/syohex/emacs-git-messenger
...
#+begin_src emacs-lisp
  (setq erc-autojoin-channels-alist
        '(
          ;; english channel
          (".*\\.freenode.net" "#emacs" "#clojure" "#wayland" "#libhybris" "#webos-ports")
          (".*\\.mozilla.org" "#b2g")
          ;; Chinese channel
...
** 使用 org-contacts 對收信人進行自動補全

emacs 下管理電子郵件位址紀錄分別有 bbdb 和 org-contacts 這兩個工具，我個人
偏好 org-contacts，因為他是屬於明碼儲存資訊，比較方便編輯。

這邊設定我預設的 org-contacts 檔案，這樣在 mu4e 中，可以使用 =a o= 將使用
者資訊加入到我的 org-contacts 中。
...
| 緩衝區   | C-x C-b | projectile-ibuffer           | 根據目前的 project 切換到 ibuffer  |
...
| C-o      | evil-execute-in-normal-state     | 暫時性切換到 Normal 模式           |
| C-d      | hungry-delete-forward            | 向右貪婪刪除                       |
| C-l      | hungry-delete-backward           | 向左貪婪刪除                       |
| <delete> | hungry-delete-backward           | 向左貪婪刪除                       |
