...
** 在目前行加上彩色條
#+begin_src emacs-lisp
  (global-set-key (kbd "C-x h") 'hl-line-mode)
#+end_src
** 顯示行號
#+begin_src emacs-lisp
...
expands them."
  ...
  (let ((l (buffer-substring (point-at-bol) (point)))
    ...
    (when (and (looking-at "[ \t]*$")
	       (string-match "^[ \t]*<\\([a-zA-Z]+\\)$" l)
	       (setq a (assoc (match-string 1 l) aa-structure-template-alist)))
      ;;      (org-complete-expand-structure-template (+ -1 (point-at-bol)
      ;;                                           (match-beginning 1)) a)
      (backward-delete-char 2)
      (insert (car (cdr-safe a)))
      (yas/expand)
      t)))


(setq aa-structure-template-alist org-structure-template-alist)

(add-to-list 'aa-structure-template-alist
      '("E" ."require"))
