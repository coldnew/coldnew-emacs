...
** 在目前的專案找尋 buffer                                         :command:

#+begin_src emacs-lisp
  (defun coldnew/project-find-buffer ()
      (interactive)
      (helm-other-buffer
       '(helm-c-source-projectile-buffers-list
         )
       "*coldnew/project-buffer-list"))
#+end_src
...
plantuml 是一個用語言寫出 UML 架構圖的程式，使用他前最好將 Garphviz 裝上。
...
;;  (add-hook 'before-save-hook 'cleanup-whitespace-before-save)
         ;; Enable line-number
         (require 'linum)
...
- evil:local-namp 綁定按鍵到 Normal 狀態下，只作用於該緩衝區
...
- evil:mode-namp 綁定按鍵到 Normal 狀態下，只作用於特定模式
...
改用 C-w 作為窗口相關的起始按鍵，原本 emacs 裡面 C-w 所使用的 kill-region

命令，改為 C-x w (或是 C-w C-w)。

#+begin_src emacs-lisp
  (evil:imap (kbd "C-w n") 'windmove-down)
  (evil:imap (kbd "C-w p") 'windmove-up)
  (evil:imap (kbd "C-w f") 'windmove-right)
  (evil:imap (kbd "C-w b") 'windmove-left)
#+end_src
...
#+begin_src emacs-lisp
  (evil:imap (kbd "C-x w") 'kill-region)
  (evil:imap (kbd "C-w C-w") 'kill-region)
#+end_src
...
*** 設定 blog 檔案的位置
#+begin_src emacs-lisp
  (defvar coldnew-blog-org-file "~/Workspace/coldnew-blog/src/blog.org"
    "coldnew's blog file")
#+end_src
*** 更新 blog 的命令
#+begin_src emacs-lisp
