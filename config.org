...
| map          | 按鍵    | 命令                | 描述                                |
|--------------+---------+---------------------+-------------------------------------|
| ibuffer-mode | C-x C-f | lusty-file-explorer | 使用 lusty-file-explorer 來尋找檔案 |

#+BEGIN_SRC emacs-lisp :noweb yes
  <<gen-keys-map(func="define-key", keys=ibuffer-keys)>>
#+END_SRC
...
| map            | 按鍵  | 命令                       | 描述              |
|----------------+-------+----------------------------+-------------------|
| company-active | C-g   | company-abort              | 離開 company-mode |
| company-active | C-n   | company-select-next        | 下一個補全        |
| company-active | C-p   | company-select-previous    | 上一個補全        |
| company-active | TAB   | company-complete-selection | 選擇該補全設定    |
| company-active | <tab> | company-complete-selection | 選擇該補全設定    |

#+BEGIN_SRC emacs-lisp :noweb yes
  <<gen-keys-map(func="define-key", keys=company-keys)>>
#+END_SRC
...
| map              | 按鍵 | 命令                                | 描述                      |
|------------------+------+-------------------------------------+---------------------------|
| minibuffer-local | M-l  | backward-kill-word                  | 刪除前一個詞              |
| minibuffer-local | M-p  | previous-history-element            | 前一個歷史紀錄            |
| minibuffer-local | M-n  | next-history-element                | 後一個歷史紀錄            |
| minibuffer-local | C-g  | minibuffer-keyboard-quit            | 離開 minibuffer           |
| minibuffer-local | C-u  | coldnew/minibuffer-clear            | 清空 minibuffer           |
| minibuffer-local | M-t  | coldnew/minibuffer-switch-tmpdir    | 切換到 /tmp 資料夾        |
| minibuffer-local | M-h  | coldnew/minibuffer-switch-homedir   | 切換到 ~/ 資料夾          |
| minibuffer-local | M-w  | coldnew/minibuffer-switch-workspace | 切換到 ~/Workspace 資料夾 |
| minibuffer-local | M-r  | coldnew/minibuffer-switch-rootdir   | 切換到 / 資料夾           |

#+BEGIN_SRC emacs-lisp :noweb yes
  <<gen-keys-map(func="define-key", keys=minibuffer-local-keys)>>
#+END_SRC
...
| Normal | <SPC> | ace-jump-mode      | 使用 ace-jump-mode 快速跳轉到符合的字 |
| Normal | M-u   | upcase-word        | 將整個英文字大寫                      |
| Normal | M-l   | downcase-word      | 將整個英文字小寫                      |
| Normal | M-c   | capitalize-word    | 將英文字字首大寫                      |
| Normal | j     | next-line          | 下一行                                |
| Normal | k     | previous-line      | 上一行                                |

#+BEGIN_SRC emacs-lisp :noweb yes :results silent
  <<gen-keys-global(func="define-key evil-normal-state-map", keys=global-evil-normal)>>
#+END_SRC

** Vim 編輯模式按鍵設定 (evil-insert-state-map)                    :noexport:
...
| Insert | C-x C-p | auto-complete       | 呼叫 auto-complete-mode 的自動補全 |
| Insert | C-/     | undo-tree-undo      | undo                               |
| Insert | M-/     | undo-tree-redo      | redo                               |
| Insert | C-x C-n | company-complete    | 呼叫 company-mode 的自動補全       |
| Insert | C-c C-e | eval-and-replace    | eval emacs-lisp 命令               |
| Insert | C-x l   | hl-line-mode        | 將游標目前行加上彩色條             |
| Insert | C-x L   | linum-mode          | 切換顯示行號                       |
| Insert | C-x C-y | yank-n-times        | 多次貼上                           |
| Insert | C-x C-l | recenter            | 將游標至於緩衝區中間               |
| Insert | C-x C-i | indent-whole-buffer | 縮排整個緩衝區                     |
| Insert | C-x C-q | toggle-read-only    | 切換緩衝區變成唯讀/可寫的狀況      |
| Insert | C-M-,   | beginning-of-buffer | 回到緩衝區開頭                     |
| Insert | C-M-.   | end-of-buffer       | 回到緩衝區尾巴                     |
