...
為了避免我的套件管理方式出錯，另外定義一個當載入套件失敗時，便會用
el-get 安裝套件的方法，注意到目前這個方案套件名稱必須和 recipes 相同。

#+BEGIN_SRC emacs-lisp
  (defmacro require* (feature &optional file)
    "Try to require FEATURE, If failed, use el-get to reinstall it."
    `(when (not (require ,feature ,file 'noerror))
       (el-get-reinstall ,feature)))
#+END_SRC
