...
*** 矩型操作
emacs以C-x r開頭的命令來進行矩形操作。先用C-space或者C-@設一個mark，移動光標到另一點，用以下命令進行列操作：
...
|         | bookmark-delete     | 刪除書籤                                   |
|         | bookmark-load       | 讀取已儲存的書籤檔案                       |
|         | bookmark-save       | 儲存書籤至 bookmark-default-file指定的地方 |
...
#+end_src
*** 使用 xcscope
emacs 和 cscope相關的套件蠻多的，xcscope 包含在 cscope 的套件裡。
...
#+begin_src emacs-lisp :tangle no
  ...
  (defconst color1 "#999")
  (defconst color2 "#555")
  (defvar arrow-right-1 (create-image (arrow-right-xpm color1 "None") 'xpm t :ascent 'center))
...
#+end_src
*** 多次貼上
emacs 本身的 yank 命令是無法使用 universla-argument的，
也就是說無法使用 C-u 80 C-y 來貼上複製的東西 80 次，
而是貼上前幾次紀錄在kill-ring的資訊。
...
#+end_src
*** eval-and-replace
將前面的 sexp 刪除並進行 eval的動作，如果eval失敗則還原前面的sexp。
#+begin_src emacs-lisp
...
#+end_src
* Google-translate
google-translate 套件提供 emacs 透過google的API進行翻譯的動作。
#+begin_src emacs-lisp
...
| 按鍵          | 命令                | 用途                 |
|---------------+---------------------+----------------------|
| Ctrl-x Ctrl-f | lusty-file-explorer | 查詢並打開檔案       |
| Ctrl-x Ctrl-y | yank-n-times        | 多次貼上             |
| Ctrl-x Ctrl-r | lusty-sudo-explorer | 使用 sudo 打開檔案   |
| Ctrl-x Ctrl-b | ibuffer             | 顯示 ibuffer         |
| Ctrl-x Ctrl-l | recenter-top-buttom | 將游標至於buffer中間 |
|               |                     |                      |
...
| 按鍵     | 命令           | 用途             |
|----------+----------------+------------------|
| Ctrl-x n | windmove-down  | 移動到下面的窗口 |
| Ctrl-x p | windmove-up    | 移動到上面的窗口 |
| Ctrl-x b | windmove-left  | 移動到左邊的窗口 |
| Ctrl-x f | windmove-right | 移動到右邊的窗口 |
...
| 按鍵       | 命令                   | 用途           |
|------------+------------------------+----------------|
| Ctrl-x g e | google-translate-zh-en | 中文翻譯成英文 |
| Ctrl-x g t | google-translate-en-zh | 英文翻譯成中文 |
#+begin_src emacs-lisp
...
| 按鍵       | 命令                     | 用途                           |
|------------+--------------------------+--------------------------------|
| Ctrl-x c c | capitalize-word-backward | 將前面的 word 第一個字母大寫   |
| Ctrl-x c l | downcase-word-backward   | 將前面的 word 變成小寫         |
| Ctrl-x c u | upcase-word-backward     | 將前面的 word 變成大寫         |
| Ctrl-x c x | dos2unix                 | 將目前的檔案轉換成unix檔案型式 |
| Ctrl-x c d | unix2dos                 | 將目前的檔案轉換成dos檔案型式  |
#+begin_src emacs-lisp
...
* Local-lisp
這邊放置我自己撰寫的 elisp package。
** mixen
:PROPERTIES:
:comments: org
#:tangle:   ~/.emacs.d/local-lisp/mixen/mixen.el
:tangle:   no
:cache: yes
:padline: no
:END:
*** TODO make a new name
提供兩種模式，一種是純粹增加空白在英文與中文的中間，但是不修改檔案
一種是儲存時自動在英文與中文中間加入空白。
*** Header
#+begin_src emacs-lisp
  ;;; mixen.el --- Add space between Chinese and English
...
**** mixen-timer
#+begin_src emacs-lisp
(defvar mixen-timer nil)
#+end_src
**** mixen-regexp
#+begin_src emacs-lisp
(defvar mixen-regexp (rx (and (group (not)))))
#+end_src
**** ch
#+begin_src emacs-lisp
  (defvar mixen-chinese-before-english-regexp
    (rx  (group (not (in "[。，！？；：「」（）、]"))
		(category chinese))
	 (group (in "a-zA-Z0-9")))
    "Regexp to find weather chinese is before english ro not.")
#+end_src

#+RESULTS[4c41d511b0fefb661e56aa230ce068ae1518afc1]:
: mixen-chinese-before-english-regexp

**** TODO bug>? en
#+begin_src emacs-lisp
  (defvar mixen-chinese-after-english-regexp
    (rx  (group (in "a-zA-Z0-9"))
	 (group (not (in "[。，！？；：「」（）、]"))
		(category chinese)))
    "Regexp to find weather chinese is after english ro not.")
#+end_src

#+RESULTS[98c4f0f0e310b3cbbc8a2c3a0c832f4418cd0243]:
: mixen-chinese-after-english-regexp

*** Functions
**** mixen-overlay-p
#+begin_src emacs-lisp
  (defun mixen-overlay-p (ov)
    "Determine whether overlay OV was created by space-between."
    (and (overlayp ov) (overlay-get ov 'mixen-overlay)))
#+end_src

#+RESULTS[0ab4fce68e6e99510dc51aa2e0962992cb095fbd]:
: mixen-overlay-p

**** mixen-start-timer
#+begin_src emacs-lisp
  (defun mixen-start-timer ()
    (setq mixen-timer (run-with-timer 1 5 'mixen-heck-buffer)))
#+end_src

#+RESULTS[460058f8db234706fd4ddf6515d45e634d455a5a]:
: mixen-start-timer

**** mixen-kill-timer
#+begin_src emacs-lisp
  (defun mixen-kill-timer ()
    (cancel-timer mixen-timer))
#+end_src
**** mixen-check-buffer
#+begin_src emacs-lisp
  (defun mixen-check-buffer ()
    "Insert a space between English words and Chinese charactors"
    (interactive)
    (mixen-delete-overlays)
    (save-excursion
      ;; (goto-char (point-min))
      ;; (while (re-search-forward mixen-chinese-before-english-regexp nil t)
       ;;  (let* ((o (mixen-make-overlay (match-beginning 1) (match-end 1))))
	;;   (overlay-put o 'after-string (concat " "))))

      (goto-char (point-min))
      (while (re-search-forward mixen-chinese-after-english-regexp nil t)
	(let* ((o (mixen-make-overlay (match-beginning 1) (match-end 1))))
	  (overlay-put o 'after-string (concat "*"))))
    ))
#+end_src
...
你 sn 你
**** mixen-region-has-mixen-overlays
#+begin_src emacs-lisp
  (defun mixen-region-has-mixen-overlays (beg end)
    "Check if region specified by BEG and END has overlay.
    Return t if it has at least one mixen overlay, nil if no overlay."
    (let ((ov (overlays-in beg end))
	  (has-mixen-overlays nil))
      (while (consp ov)
	(when (mixen-overlay-p (car ov))
	  (setq has-mixen-overlays t))
	(setq ov (cdr ov))
	has-mixen-overlays)))
#+end_src

#+RESULTS[6835878c6c755159365fa3e506a2696ee1f76cc3]:
: mixen-region-has-mixen-overlays

**** mixen-make-overlay
#+begin_src emacs-lisp
  (defun mixen-make-overlay (beg end)
    "Allocate a mixen overlay in range."
    (when (not (mixen-region-has-mixen-overlays beg end))
      (let ((ov (make-overlay beg end nil t t)))
	(overlay-put ov 'mixen-overlay t)
	(overlay-put ov 'priority 100)
	ov)))
#+end_src

#+RESULTS[d2026b8ada718fbd22bfef11c3770b577e8363b0]:
: mixen-make-overlay

*** Commands
**** mixen-delete-overlays
#+begin_src emacs-lisp
  (defun mixen-delete-overlays ()
    "Delete all mixen-overlays in BUFFER."
    (interactive)
    (dolist (ov (overlays-in (point-min) (point-max)))
      (when (mixen-overlay-p ov)
	(delete-overlay ov))))
#+end_src

#+RESULTS[19dabdfdec9df934f739b9bd5673e86adbf20a23]:
: mixen-delete-overlays
