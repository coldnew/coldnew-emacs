...
| C-x t j <NUM> | elscreen-goto            | 跳到 <NUM> 頁 |
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-x t j") 'elscreen-goto)
...
| 按鍵  | 命令                | 用途           |
|-------+---------------------+----------------|
| C-x i | indent-whole-buffer | 縮排整個緩衝區 |

#+begin_src emacs-lisp
  (global-set-key (kbd "C-x i") 'indent-whole-buffer)
#+end_src
