...
define(`transformer', `[ P1: Here
 ...
 `$1' )
 P2: Here
  inductor(from ifelse(`$2',R,`P2 to P1',`P1 to P2'),m4WL,`$3')
 ...
 S1: 2 between S2 and Here
  inductor(from ifelse(`$2',R,`S1 to S2',`S2 to S1'),m4WL,`$5')
...
define(`dna_',`$5')dnl
 ...
 { sc_draw(`dna_',T,
    `line to N then to 0.75 between N and E then to 0.75 between S and E \
       then to S then to Here; line from 0.75 between Here and E to E',
    `line to N then to E then to S then to Here; move to E')
   if rp_len > m4h then { line to rvec_(rp_len-m4h,0) }
 Out: Here }
 E1: rvec_(m4h/2,m4v/4)
 E2: rvec_(m4h/2,-m4v/4)
 In1: rvec_(0,m4v/4)
 In2: rvec_(0,-m4v/4)
 ...
 sc_draw(`dna_',P,
   `{line from E1 to rvec_(m4h/2,m4v/4+m4v/8);    V1: Here}
    {line from E2 to rvec_(m4h/2,-(m4v/4+m4v/8)); V2: Here}')
 `$6' ] ')
...
M4_s: last line.start; M4_e: last line.end
  ...
  sc_draw(`ddna_',WW,dnl            Thyristor gate
  ...
  sc_draw(`ddna_',W,dnl            Thyristor gate
   `{line from rvec_(0,ifelse(m4a,d,-)m4dv/4) \
                     to rvec_(m4dh,ifelse(m4a,d,-)(m4dv*3/4))
     G: Here}')
   ...
   `arc ifelse(m4x,2,c)cw to rrot_(dimen_/10,0,ang) \
    ...
    arrow to `$1'.end wid em_arrowwid ht em_arrowht*3/4 }')
                                ...
                                `thyristor(linespec, chars)
                                 chars:
                                   D: Diode
                                   A: Open diode, UA=open diode with centre line
                                   B: Bidirectional diode
                                   C: Type IEC
                                   G: Full-size gate terminal
                                   R: Right orientation
                                   E: envelope
                                 To do: if linespec ~= blank, draw as a
                                    two-terminal element, or maybe use name scr'
define(`thyristor',`ifinstr(`$2',C,
 `bi_trans(`$1',ifinstr(`$2',R,,R),uEdCBUT`'xtract(`$2',E),,A:E;K:C;G:T;`$3')',
 `ifinstr(`$2',B,
   `[define(`m4v',`dimen_/5*2')define(`m4h',`sqrt(3)*dimen_/5/2')dnl
    T1: Here
    eleminit_(`$1',8*dimen_/10)
    T2: last line.end
    ifinstr(`$2',R,`m4gen_d(LdG'xtract(`$2',G)`dFHuQR)',
                   `m4gen_d(LG'xtract(`$2',G)`uFHdQR)')
    ifinstr(`$2',E,`Env:circle rad 4*dimen_/10 with .c at rvec_(rp_len/2,0)')
    `$3']',
   `[ eleminit_(`$1',dimen_*0.7)
      A: Here
      K: last line.end
      m4gen_d(LC'ifinstr(`$2',A,
       ifinstr(`$2',UA,Ac)A,F)`'ifinstr(`$2',R,d)`'ifinstr(`$2',E,W)`WR)
      ifinstr(`$2',E,`Env:circle diam dimen_*0.7 at 0.5 between A and K')
      `$3']')')')
...
define(`tgate',`[ eleminit_(`$1') define(`m4tgm',ifinstr(`$2',L,-))
 ...
 Gb: C+vec_(0,m4tgm`'m4ht/2)
    line from 2 between Gb and C to 4 between Gb and C',
   `m4gen_d(uLAVuR)
    circle thick max(4pt__,linethick/2) rad m4dh/4 \
      at C+vec_(0,m4tgm`'m4dh/4*4/3)
    line from last circle+vec_(0,m4tgm`'m4dh/4) to C+vec_(0,m4tgm`'m4dh*3/2)
 Gb: Here
    line from C to C-vec_(0,m4tgm`'m4dh) ')
 G: Here
  `$3']')
...
define(`ptrans',`[ eleminit_(`$1') define(`m4ptm',ifinstr(`$2',L,-))
 ...
 B: last line.end
 C: last line.center
    line to rvec_(rp_len/2-m4pwd/2,0)
    { line to rvec_(m4pwd,m4pv) then to rvec_(m4pwd,neg_(m4pv)) then to Here }
    { line from rvec_(m4pwd,0) to rvec_(0,m4pv) then to rvec_(0,neg_(m4pv)) \
        then to rvec_(m4pwd,0)
    circle thick max(4pt__,linethick/2) rad m4pv/4 at C+vec_(0,m4ptm`'m4pv*5/6)
    line from last circle+vec_(0,m4ptm`'m4pv/4) to C+vec_(0,m4ptm`'m4pv*2)
 Gb:Here
    line from C-vec_(0,m4ptm`'m4pv/2) to C-vec_(0,m4ptm`'m4pv*3/2)
 G: Here }
    line from rvec_(m4pwd,0) to B
  `$3']')
...
define(`Darlington',
 `[Q1: bi_trans(`$1',`$2',ifelse(`$3',P,u,d)EBCBU)
   ...
   Q2: bi_trans(`$1',`$2',ifelse(`$3',P,u,d)EBCBU) with .E at Q1.B
   B: Q2.B
   T: Q2.C+(Q2.Bulk.x-Q2.B.x,Q2.Bulk.y-Q2.B.y) 
   C: intersect_(Q2.C,T,Q1.E,Q1.C)
   line from Q2.C to C then to Q1.C
   `$4' ]')
...
[ ifelse(`$1',,`tr_xy_init(,m4_U,m4R); E: tr_xy(-3,0); C: tr_xy(3,0)',
  ...
  sc_draw(`dna_',S,
   `Bulk: line invis from Bulk.start+ta_xy(-1,0) to Bulk.end+ta_xy(1,0)
    line from Bulk.end+ta_xy(-1,0) to Bulk.end \
      then to Bulk.end+ta_xy(0,-0.5) then to Bulk.end+ta_xy(-0.5,-0.5)
    line from Bulk.start+ta_xy(1,0) to Bulk.start \
...
define(`c_fet',`mosfet(`$1',`$2',`ZSDF'ifelse(`$3',P,d)T,,`$4')')
 ...
 ` The comprehensive mosfet(linespec,R,BDEFGLQRSTXZ,E)
   ...
   or letter sequence:
                               ...
                               udG: G pin to substrate at source
                               udH: G pin to substrate at center
                                 L: G pin to channel
...
define(`mosfet',
 `define(`m4R',`ifelse(`$2',R,-)')dnl
  ...
  define(`m4s',ifinstr(dna_,Z,2.5,3.5))dnl
[ ifelse(`$1',,`tr_xy_init(,m4_U,m4R); S: tr_xy(-2,0); D: tr_xy(2,0)',
   `eleminit_(`$1'); tr_xy_init(last line.c,m4_U,m4R)
  ...
  sc_draw(`dna_',H,
   `G: tr_xy(0,(m4s+4))
    ifelse(m4a,,`line from tr_xy(0,m4s) to G',
           m4a,d,`arrow from G to tr_xy(0,m4s) ht m4_ht wid m4_wd',
           m4a,u,`line from tr_xy(0,m4s) to G; arrow ht m4_ht wid m4_wd \
  ...
  sc_draw(`dna_',Pz,
   `define(`m4q',m4a)dnl
    ...
    line from tr_xy(-2,-2) to tr_xy(-2,0)
    line from tr_xy( 2,-2) to tr_xy( 2,0)')
  sc_draw(`dna_',Q,
   `line from tr_xy(0,0)+vec_(0,m4R`'linethick pt__) to tr_xy(0,0) \
      then to tr_xy(-2,0) then to tr_xy(-2,0)+vec_(0,m4R`'linethick pt__)')
  sc_draw(`dna_',R,
   `line thick 2*linethick from tr_xy(-2,m4s)-vec_(0,m4R`'linethick*3/2 pt__) \
  ...
  sc_draw(`dna_',X,dnl          From Matteo Agostinelli
   `B: tr_xy(0,0); line from B to tr_xy(0,m4s-1)
       line from tr_xy(-1.5,m4s-1) to tr_xy(1.5,m4s-1)')
...
define(`speaker',`[direction_($1)
 ...
 ifelse(`$3',H,
  `{line from rvec_(m4h,m4v/2) to rvec_(m4h*3/2,m4v*7/8) \
...
define(`earphone',`[direction_($1)
  ...
  ifinstr(`$3',C,
   ...
   `{lbox(m4h/3,m4h/2)}
    ...
    { line thick 2*linethick from rvec_(m4h/3+linethick pt__,-m4h/3) to \
