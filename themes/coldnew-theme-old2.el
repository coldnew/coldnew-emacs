;;; coldnew-theme-old2.el --- old coldnew's emacs color-theme.
;;
...
(defvar coldnew-theme-colors-old2
  '( )
...
(defun coldnew-theme--build-colors-alist-old2 (mode)
  ...
  (noflet ((find-color (color)
    ...
    (mapcar (lambda (x) (list (symbol-name (car x)) (find-color x)))
            (cdr (assoc (cadr `,mode) coldnew-theme-colors-old2)))))

(defmacro coldnew-theme--with-colors-old2 (mode &rest body)
  "`let' bind all colors defined in `coldnew-theme-colors-old2' around BODY.
Also bind `class' to ((class color) (min-colors 89))."
  (declare (indent 0))
  `(let ((class '((class color) (min-colors 89)))
         ,@(mapcar (lambda (cons)
                     (list (intern (car cons)) (cadr cons)))
                   (coldnew-theme--build-colors-alist-old2 mode)))
     ,@body))

(defmacro coldnew-theme--face-specs-old2 ()
  (quote
...
(defun coldnew-theme--theme-name-old2 (mode)
  (intern (format "coldnew-theme-%s" (symbol-name mode))))

(defmacro coldnew-theme--define-theme-old2 (mode)
  "Define a theme for the coldnew variant `MODE'."
  (let ((name (coldnew-theme--theme-name-old2 mode))
    ...
    `(progn
       ...
       (coldnew-theme--with-colors-old2
         ',mode
         (apply 'custom-theme-set-faces ',name
                (coldnew-theme--face-specs-old2))
...
(defun coldnew-theme--load-theme-old2 (mode)
...
(provide 'coldnew-theme-old2)

;; Local Variables:
;; byte-compile-warnings: (not cl-functions)
;; End:

;;; coldnew-theme-old2.el ends here
