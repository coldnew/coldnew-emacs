  ...
  (mapc
   (lambda (entry)
	...
	(dictree--cache-results (cdr entry)))
      ...
      cache))
   (dictree--complete-ranked-cache dict-cpp))
  (setf (dictree--complete-ranked-cache dict-cpp)
	cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-cpp)))
  (mapc
   (lambda (entry)
	...
	(dictree--cache-results (cdr entry)))
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-cpp))
  (setf (dictree--regexp-ranked-cache dict-cpp)
	cache))
(unless (memq dict-cpp dictree-loaded-list)
  (push dict-cpp dictree-loaded-list))
