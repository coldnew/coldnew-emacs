...
define(`DrawIn',
 `LastInput: LastInput+(inputsep,0)
  In`'$1: LastInput
  "$$1$" ljust at LastInput     # Maybe labels should be done externally
  Int`'$1: LastInput
  define(D`'$1)')
...
[ CanLogic(AND,,OR,,abcd,a~b,c,~ad) #; line right jog from Out "$f$" above
  ]
{`"\tt CanLogic(AND,,OR,,abcd,a\char126{}b,c,\char126{}ad)"' \
    at last [].s -(0,11bp__)}

[ CanLogic(OR,N,NAND,,ab~c,a~bc,ac,~d) #; line right jog from Out "$f$" above
  ] with .sw at last [].se+(0.5,0)
{`"\tt
  CanLogic(OR,N,NAND,,ab\char126{}c,a\char126{}bc,ac,\char126{}d)"' \
    at last [].s - (0,11bp__)}
