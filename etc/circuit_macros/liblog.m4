...
ifdef(`sourcerad_',,`include(HOMELIB_`'libcct.m4)divert(-1)')
...
define(`AND_gen',
 ...
 [ sc_draw(`dna_',B,dnl
   `Bm: line from svec_(m4h-m4v/2,-m4v/2) to svec_(0,-m4v/2) \
     then to svec_(0,m4v/2) then to svec_(m4h-m4v/2,m4v/2)')
...
define(`OR_gen',
 ...
 [sc_draw(`dna_',P,`define(`m4o',XOR_off*m4v/(AND_ht))dnl
    Parc: arc cw from svec_(0,m4v/2) to svec_(0,-m4v/2) \
      with .c at svec_(-sqrt((OR_rad*m4v/(AND_ht))^2-(m4v/2)^2),0)')
  sc_draw(`dna_',B,dnl
   `Bt: line from svec_(m4o+m4h/3,m4v/2) to svec_(m4o,m4v/2)
    ArcB: arc cw to svec_(m4o,-m4v/2) \
      with .c at svec_(m4o-sqrt((OR_rad*m4v/(AND_ht))^2-(m4v/2)^2),0)
    Bb: line to svec_(m4o+m4h/3,-m4v/2)')
...
define(`BUFFER_gen',
 ...
 [sc_draw(`dna_',T,
   `Tm: line from svec_(m4h,0) to svec_(0,-m4v/2) then to svec_(0,m4v/2) \
     then to svec_(m4h,0)')
  sc_draw(`dna_',NNE,
...
define(`Mux',`[
Chip: [LChip: line from svec_(-Mux_wid/2, 0) to svec_(-Mux_wid/2, Mux_ht/2) \
